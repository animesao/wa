...
- **Исправление:** Эффекты зелий из сумки больше не ломаются при снятии брони/левой руки с тем же типом эффекта
- **Исправление:** Добавлена карта `trackedComponents` для правильной очистки всех предыдущих эффектов перед пересканированием
- **Исправление:** `onUnequip` теперь вызывается для всех отслеживаемых компонентов перед пересчётом
- **Рефакторинг:** `ArmorListener` упрощён до единого метода `checkEquipmentChange()` на основе сравнения слотов
- **Рефакторинг:** Удалены все отдельные вызовы `onEquip`/`onUnequip` — всё проходит через `recalcEffects`
- **Исправление:** Добавлен обработчик `PlayerRespawnEvent` для сброса отслеживания и повторного применения эффектов после смерти
- **Изменение:** `scanOnStartup` по умолчанию `false` — лут подземелий генерируется лениво при первом открытии сундука
- **Функция:** bStats метрики с кастомными графиками (тип БД, количество артефактов)
- **Инфра:** Миграция на Shadow плагин для сборки

### v2.3.0 — Исправления ошибок и расширение компонентов
- **Исправление:** NPE при ошибке подключения к БД
- **Функция:** Добавлен компонент `POTION_EFFECT_ON_HIT` (накладывает эффект зелья на цель при атаке)
- **Функция:** Добавлен компонент AURA (периодический эффект зелья в радиусе)
- **Разное:** Удалён плейсхолдер скриншотов из конфига
...
- Сканирует все миры на предмет подземелий (если `scanOnStartup: true` — теперь `false` по умолчанию; лут генерируется лениво при первом открытии сундука)
...
| `scanOnStartup` | Boolean | `false` | Сканировать все миры на предмет структур при запуске (ленивая генерация при открытии сундука) |
...
> **Wasteland Artifacts v2.4.0** — Создано animesao для Paper 1.21.11+.
