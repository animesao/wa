...
public class AltarCommand extends Command {
    ...
    public boolean execute(@NotNull CommandSender sender, @NotNull String label, @NotNull String[] args) {
        if (!(sender instanceof Player player)) {
            ComponentUtil.sendMsg(sender, plugin.msg("altar.players-only"));
            return true;
        }
        ...
        return switch (args[0].toLowerCase()) {
            ...
            case "build" -> {
                if (!player.hasPermission("wastelandartifacts.admin.altar")) {
                    ComponentUtil.sendMsg(player, plugin.msg("altar.no-permission-preview"));
                    yield true;
                }
            ...
            case "schematic" -> {
                if (!player.hasPermission("wastelandartifacts.admin.altar")) {
                    ComponentUtil.sendMsg(player, plugin.msg("altar.no-permission"));
                    yield true;
                }
    ...
    private void sendHelp(Player player) {
        ComponentUtil.sendMsg(player, plugin.msg("altar.help-title"));
        ComponentUtil.sendMsg(player, plugin.msg("altar.help.list"));
        ComponentUtil.sendMsg(player, plugin.msg("altar.help.info"));
        ComponentUtil.sendMsg(player, plugin.msg("altar.help.preview"));
        if (player.hasPermission("wastelandartifacts.admin.altar")) {
            ComponentUtil.sendMsg(player, plugin.msg("altar.help.build"));
            ComponentUtil.sendMsg(player, plugin.msg("altar.help.schematic"));
        } else {
            ComponentUtil.sendMsg(player, plugin.msg("altar.help.preview-hint"));
        }
    }
    ...
    private boolean handleList(Player player, AltarManager mgr) {
        ...
        if (tiers.isEmpty()) {
            ComponentUtil.sendMsg(player, plugin.msg("altar.list-empty"));
            return true;
        }
        ComponentUtil.sendMsg(player, plugin.msg("altar.list-title"));
        for (var entry : tiers.entrySet()) {
            AltarConfig.AltarTier t = entry.getValue();
            if (!t.enabled) continue;
            ComponentUtil.sendMsg(player,
    ...
    private boolean handleInfo(Player player, AltarManager mgr, String[] args) {
        if (args.length < 2) {
            ComponentUtil.sendMsg(player, plugin.msg("altar.usage.info"));
            return true;
        }
        AltarConfig.AltarTier tier = mgr.getTier(args[1]);
        if (tier == null) {
            ComponentUtil.sendMsg(player, plugin.msg("altar.not-found", args[1]));
            return true;
        }
        ComponentUtil.sendMsg(player, plugin.msg("altar.info-title", tier.displayName));
        ComponentUtil.sendMsg(player, plugin.msg("altar.info.tier", tier.tier));
        ComponentUtil.sendMsg(player, plugin.msg("altar.info.description", tier.description));
        ComponentUtil.sendMsg(player, plugin.msg("altar.info.activator", tier.activatorBlock.name()));
        ComponentUtil.sendMsg(player, plugin.msg("altar.info.cooldown", tier.globalCooldown));
        ComponentUtil.sendMsg(player, plugin.msg("altar.info.structures",
                tier.structures != null ? tier.structures.size() : 0));

        if (tier.recipes != null && !tier.recipes.isEmpty()) {
            ComponentUtil.sendMsg(player, plugin.msg("altar.info.recipes-title"));
    ...
    private boolean handlePreview(Player player, AltarManager mgr, String[] args) {
        if (args.length < 2) {
            ComponentUtil.sendMsg(player, plugin.msg("altar.usage.preview"));
            return true;
        }

        if ("stop".equals(args[1])) {
            mgr.getPreview().stopPreview(player);
            ComponentUtil.sendMsg(player, plugin.msg("altar.preview-stopped"));
            return true;
        }
    ...
    private boolean handleBuild(Player player, AltarManager mgr, String[] args) {
        if (args.length < 2) {
            ComponentUtil.sendMsg(player, plugin.msg("altar.usage.build"));
            return true;
        }
        AltarConfig.AltarTier tier = mgr.getTier(args[1]);
        if (tier == null) {
    ...
    private boolean handleSchematic(Player player, AltarManager mgr, String[] args) {
        if (args.length < 2) {
            ComponentUtil.sendMsg(player, plugin.msg("altar.usage.schematic"));
            return true;
        }
        ...
        return switch (args[1].toLowerCase()) {
            case "save" -> {
                if (args.length < 3) {
                    ComponentUtil.sendMsg(player, plugin.msg("altar.usage.schematic-save"));
                    yield true;
                }
            ...
            case "paste" -> {
                if (args.length < 3) {
                    ComponentUtil.sendMsg(player, plugin.msg("altar.usage.schematic-paste"));
                    yield true;
                }
            ...
            case "list" -> {
                ...
                if (list.isEmpty()) {
                    ComponentUtil.sendMsg(player, plugin.msg("altar.schematic-list-empty"));
                } else {
                    ComponentUtil.sendMsg(player, plugin.msg("altar.schematic-list", String.join(", ", list)));
                }
                yield true;
            }
            default -> {
